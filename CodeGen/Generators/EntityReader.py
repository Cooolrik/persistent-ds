...
def EntityReader_h():
	...
	lines.append('    status_return<EntityReader*> BeginReadSectionsArray( const char *key, const u8 key_length, const bool null_object_is_allowed, std::vector<i32> *dest_index = nullptr );')
	...
	for basetype in hlp.base_types:
		...
		for type_impl in basetype.variants:
			...
			if type_impl.overrides_type:
				...
				lines.append(f'template <> status EntityReader::Read<std::vector<{implementing_type}>>( const char *key, const u8 key_length, std::vector<{implementing_type}> &dest_variable )')
				lines.append(f'{{')
