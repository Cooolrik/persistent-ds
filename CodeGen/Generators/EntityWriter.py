...
def EntityWriter_h():
	...
	lines.append('\tstatus_return<EntityWriter*> BeginWriteSectionsArray( const char *key, const u8 key_length, const size_t array_size, const vector<u32> *index = nullptr );')
	...
	for basetype in hlp.base_types:
		...
		for type_impl in basetype.variants:
			...
			if type_impl.overrides_type:
				...
				lines.append(f'// {implementing_type}: using vector<{item_type}> to store' )
				lines.append(f'template <> status EntityWriter::Write<vector<{implementing_type}>>( const char *key, const u8 key_length, const vector<{implementing_type}> &src_variable )')
				lines.append(f'{{')
				...
				lines.append(f'\treturn this->Write<vector<{item_type}>>( key, key_length, tmp_variable );')
				lines.append(f'}}')
				lines.append(f'')
				...
				lines.append(f'//  {array_type_name}: vector<{implementing_type}>' )
				lines.append(f'template <> status EntityWriter::Write<vector<{implementing_type}>>( const char *key, const u8 key_length, const vector<{implementing_type}> &src_variable )')
				lines.append(f'{{')
				...
				lines.append(f'template <> status EntityWriter::Write<optional_vector<{implementing_type}>>( const char *key, const u8 key_length, const optional_vector<{implementing_type}> &src_variable )')
				lines.append(f'{{')
				lines.append(f'\tconst vector<{implementing_type}> *p_src_variable = (src_variable.has_value()) ? &(src_variable.values()) : nullptr;')
				...
				lines.append(f'template <> status EntityWriter::Write<optional_idx_vector<{implementing_type}>>( const char *key, const u8 key_length, const optional_idx_vector<{implementing_type}> &src_variable )')
				lines.append(f'{{')
				lines.append(f'\tconst vector<{implementing_type}> *p_src_values = (src_variable.has_value()) ? &(src_variable.values()) : nullptr;')
				lines.append(f'\tconst vector<u32> *p_src_index = (src_variable.has_value()) ? &(src_variable.index()) : nullptr;')
