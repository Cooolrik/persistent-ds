...
# Warning! This will not fix all errors, and the files will still need to be checked manually
fix_fails = False
...
license = "Licensed under the MIT license https://github.com/Cooolrik/pds/blob/main/LICENSE"
...
def check_file(file_path: Path) -> bool:
	...
	defs.extend( [
		...
		line_define(next_row + 1, comment + license ),
	] )
	next_row += 2
	...
	fails = check_file_lines(file_path, defs)
	if len(fails) > 0:
		if autogen or not fix_fails:
			print(f"File: {file_path} fails header check: {fails}")
			return False
		else:
...
if __name__ == "__main__":
	...
	errors = []
	source_directories = ["include/pds","tests"]
